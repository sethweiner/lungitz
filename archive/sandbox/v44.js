...
function scrollToTrigger(trigger, instant) {
    var col = trigger.closest('.wrapper-content'),
        pad = 64, DUR = 620, scroller, docScroll, from, start = null;
...
(function infoHashArrival() {
    ...
    if (!el) { return; }
    // Re-settle until the page stops moving. One correction at load is not enough:
    // fonts, images and CMS lists each land at their own pace, and every one of them
    // that reflows ABOVE the target shifts it again — measured 80px of accumulated
    // drift deep in the stacked mobile layout, which is why a single fonts.ready pass
    // still left the info text sitting low. Re-apply over the first second and a half,
    // instantly each time (arrive()'s precedent: an arrival corrects, it does not
    // perform). Idempotent once the layout is stable, and any real scroll or keypress
    // cancels the rest so it can never yank a reader who has started reading.
    ...
    [0, 150, 400, 800, 1400].forEach(function (d) {
