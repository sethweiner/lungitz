...
(function injectCSS() {
    var INK = 'var(--_lungitz---color-ink-900)',
        css = [
            // MODAL FITS THE FRAME (v38 — Seth: "the modal goes off the page").
            // ONLY the height cap lives here, because it is the one part that must
            // be scoped to the open state: give the RESTING word row a height and
            // it becomes a full-screen invisible block sitting over the index.
            // A descendant selector is the only way to say that, so contract §2.
            // The frame inset is the panel's own margin:16px, hence 100vh - 32px.
            // The rest of the fix is on the classes themselves, in the Designer:
            //   · .landing-content dropped its hard height:88vh — that was the
            //     overflow (88vh + word row + menu row + gaps + padding = the 716px
            //     measured in a 720 viewport) — and gained min-height:0, without
            //     which a 1fr track refuses to shrink below its content.
            //   · .nav.wide.landing declared ONE explicit row, so both the content
            //     and the menu row landed in implicit tracks; now auto 1fr auto.
            '.container-landing.is-active .nav.wide.landing{',
...
function scrollToTrigger(trigger) {
    var col = trigger.closest('.wrapper-content'),
        pad = 64, DUR = 620, from, start = null;
    if (!col) {
        return;
    }
    from = col.scrollTop;
    function aim() {                       // absolute destination, stable as we scroll
        return Math.max(0, col.scrollTop
    ...
    function step(now) {
        ...
        col.scrollTop = from + (aim() - from) * eased;
