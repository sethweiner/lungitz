...
function scrollToTrigger(trigger, instant) {
    ...
    // Frame-rate independent (time decides, not frame count), starts from zero
    // velocity (no teleport), still re-reads the destination every frame, and the
    // last frame takes portion 1, so it lands exactly as before.
    var prevE = 0;
    function ease(p) {          // easeInOutCubic — candidate feel, Seth's call (T-07)
