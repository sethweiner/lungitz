...
// scroll-padding-top is what a native #hash navigation honours, and it was pinned at
// 64px in the head block — a desktop constant. The masthead is 39px tall at 375px, so
// the browser parked info blocks at the wrong offset no matter what the script did
// afterwards. Publish the measured pad onto html (and refresh it when the viewport
// changes) so the native route and scrollToTrigger agree at every breakpoint.
(function syncScrollPadding() {
    function apply() {
        try {
            var pad = anchorPad() + 'px';
            // BOTH scrollers. The head block pins 64px on html AND .wrapper-content;
            // whichever one is doing the scrolling at this breakpoint is the one the
            // browser consults, so updating only html left the desktop columns on the
            // stale constant. The masthead is not the same height at every width — it
            // measured 41px at one size and 58px at another — so this has to be read,
            // never assumed, and re-read whenever the viewport or the type changes.
