...
// [sandbox v31] production's /sandbox bail is removed so this standalone build runs here.
// v31 — NAV CONCEPT REDESIGN (17.6 feedback) + landing veil + mobile fixes:
//   · GIVEAWAYS/HIDEAWAYS words now GO TO their realm (scroll + rust-light), not the menu
...
(function navMenu() {
    ...
    var V = function (n) { return 'var(--_lungitz---' + n + ')'; },
        css = [
            ...
            // v31: breakpoint unified with the masthead's fixed-position block (767,
            // durabilityPolish) — the menu stacked at 640 while the nav went fixed at
            // 767, leaving a 641–767 band with a fixed nav and an unstacked menu.
            ...
            // v31 — the Impressum trap: the fixed mobile masthead had no height cap and
            // .nav-body clips (overflow:hidden), so an open drawer taller than the screen
            // was simply unreachable. Cap the OPEN drawer to the dynamic viewport (dvh —
            // iOS URL-bar-safe; 1.5rem = 2 × the 0.75rem fixed margin) and let the body
            // row scroll with momentum, without chaining the page behind it. overflow-y
            // still clips during the 0fr collapse, so the drawer motion is unaffected.
            '  ' + NAVC + '.is-open{max-height:calc(100dvh - 1.5rem);}',
            '  ' + NAVC + '.is-open .nav-body{overflow-y:auto;-webkit-overflow-scrolling:touch;overscroll-behavior:contain;}',
            // realm-word arrival: keep the column start clear of the fixed masthead
    ...
    if (cmsMenu && cmsMenu.querySelector('.nav-item')) {
        ...
        nav.querySelectorAll('.nav-item').forEach(function (el) {
            el.addEventListener('click', function (e) {
                // Nav redesign (v31): a Designer-linked item (real href — bound to its
                // CMS template page) NAVIGATES like a normal link. The inline reveal
                // below only runs for unlinked items (fallback until Seth binds them).
    ...
    // Clicking GIVEAWAYS / HIDEAWAYS takes you to that column (scroll + the rust
    // realm cue, transient like the ?entry= arrival) instead of opening the menu.
    // Off the index (entry/menu pages, no columns) it navigates home flagged
    // (?realm=side) and the wayfinding module finishes the arrival.
    ...
    function goToRealm(side) {
        var col = document.querySelector('.wrapper-content.' + (side === 'hideaways' ? 'is-right' : 'is-left'));
    ...
    // The menu now opens from a dedicated control: the Designer-owned
    // .nav-menu-toggle (Seth authors + styles it in the masthead component).
    // Interim fallback until it exists: LUNGITZ toggles the menu on the index
    // (off the index LUNGITZ means "home" — wayfinding owns it there).
    var gWord = nav.querySelector('.nav-giveaways'),
        hWord = nav.querySelector('.nav-hideaways'),
        lWord = nav.querySelector('.nav-lungitz'),
        menuToggle = nav.querySelector('.nav-menu-toggle'),
        onIndex = !!document.querySelector('.wrapper-content.is-left, .wrapper-content.is-right');
    if (gWord) { gWord.addEventListener('click', function (e) { e.preventDefault(); goToRealm('giveaways'); }); }
    if (hWord) { hWord.addEventListener('click', function (e) { e.preventDefault(); goToRealm('hideaways'); }); }
    if (menuToggle) {
    ...
    if (lWord) {
        if (!menuToggle && onIndex) {
            lWord.style.cursor = 'pointer';
            lWord.addEventListener('click', function (e) { e.preventDefault(); toggleMenu(); });
        } else {
...
(function wayfinding() {
    ...
    // v31 — any OTHER page without the index columns (the Menu Entries template
    // pages: /impressum, /resources, … — the nav-redesign "submenus are real
    // pages") gets the same way back: LUNGITZ returns home. Plain '/' (no ?entry
    // flag — there's no entry to arrive at).
    if (!document.querySelector('.wrapper-content.is-left, .wrapper-content.is-right')) {
        var lw = document.querySelector('.nav-lungitz');
        if (lw) {
    ...
    // v31 — ?realm=giveaways|hideaways arrival (the realm words' cross-page path):
    // scroll the column up and light its masthead word, transient like ?entry=.
    var realmFlag = /[?&]realm=(giveaways|hideaways)\b/.exec(location.search);
    if (realmFlag) {
        var rSide = realmFlag[1];
        var goRealm = function () {
            var col = document.querySelector('.wrapper-content.' + (rSide === 'hideaways' ? 'is-right' : 'is-left'));
            if (!col) { return; }
            col.scrollIntoView({ block: 'start' });   // instant — same view-transition logic as arrive()
            col.scrollTop = 0;
            if (typeof lightRealm === 'function') { lightRealm(rSide); }
            var clearRealm = function () {
                if (typeof lightRealm === 'function') { lightRealm(null); }
                document.removeEventListener('pointerdown', clearRealm, true);
                window.removeEventListener('wheel', clearRealm, true);
            };
            document.addEventListener('pointerdown', clearRealm, true);
            window.addEventListener('wheel', clearRealm, { capture: true, passive: true });
            if (history.replaceState) { history.replaceState({}, '', location.pathname); }
        };
        if (document.readyState === 'loading') { document.addEventListener('DOMContentLoaded', goRealm); }
        else { goRealm(); }
        return;
    }
...
(function durabilityPolish() {
    ...
    var css = [
        ...
        '}',
        // v31: the ✕-less fullscreen (LUNGITZ/backdrop/Esc close) assumed a keyboard
        // and a cursor that can read the exit affordances. On touch there is neither —
        // 17.6 feedback: "how to get back without Esc?". Keep the clean frame ONLY for
        // fine pointers; coarse pointers get the Designer-owned .frame-close back
        // (its look + the .is-immersive reveal motion are already wired above).
        '@media (hover:hover) and (pointer:fine){.frame-close{display:none!important;}}',
...
// ════════════════════════════════════════════════════════════════════════
//  §Landing veil (v31 — 17.6 "Landing page") — an intro moment OVER the index,
//  not a page before it (moving Home off / would break wayfinding, the entry-nav
...
(function landingVeil() {
    ...
    function dismiss() {
        ...
        setTimeout(function () { veil.remove(); }, 750);
    }
    function onKey(e) {
        if (done) { return; }
        if (e.key === 'Enter' || e.key === 'Escape' || e.key === ' ') {
            e.preventDefault();
            e.stopPropagation();   // capture on document — the ladder's handlers never see it
            dismiss();
        }
    }
