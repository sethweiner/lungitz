...
// Touch/pen — anything without a precise hovering pointer. Used to keep gestures
// and scroll-follow behaviour apart, never to change what anything looks like.
...
document.addEventListener('click', function (e) {
    ...
    // COARSE POINTERS DO NOT GET THE SCROLL-FOLLOW (v48). Tweening the page for
    ...
    // does not, and the follow is the point. On touch we let the entry open in place
    // and settle it once, instantly, after the transition has finished.
    if (hadOpen) {
        if (!coarsePointer()) { scrollToTrigger(trigger); }
        pendingOpen = setTimeout(function () {
            pendingOpen = null;
            trigger.classList.add('open', 'is-engaged');
            if (coarsePointer()) { settleAfterExpand(trigger); }
        }, CLOSE_STAGGER);
    } else {
        trigger.classList.add('open', 'is-engaged');
        if (coarsePointer()) { settleAfterExpand(trigger); }
