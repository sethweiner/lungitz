...
// ── §State 4b : Scroll-to-zoom (v12 — replaces click-to-zoom in fullscreen) ──
// Proven on the gesture catalog, ported here. Wheel down zooms in toward the
// cursor (clamped at the image's natural scale, capped 4×); wheel up zooms back
// out, and at 1× it releases so the page scrolls again. Origin is anchored where
// the zoom began; pan (drag) keeps working while zoomed. State drawers stay
...
document.addEventListener('wheel', function (e) {
    ...
    if (!zoom) {
        if (!inDir) { return; }                       // not zoomed + scrolling out → page scrolls
        rect = img.getBoundingClientRect();
        natS = Math.max(img.naturalWidth / rect.width, img.naturalHeight / rect.height);
        zoom = { scale: 1, panX: 0, panY: 0, max: Math.max(1.5, Math.min(natS, 4)) };
        img.style.transformOrigin =
            Math.max(0, Math.min(100, (e.clientX - rect.left) / rect.width * 100)) + '% ' +
            Math.max(0, Math.min(100, (e.clientY - rect.top) / rect.height * 100)) + '%';
        img.classList.add('is-zoomed');
    }
    if (inDir && zoom.scale >= zoom.max) { return; }  // at max → let the event go
    e.preventDefault();
    zoom.scale = Math.max(1, Math.min(zoom.max, zoom.scale + e.deltaY * 0.0045));
    if (zoom.scale <= 1.001) { zoomOut(img); return; }
    img.style.transition = 'transform 90ms linear';
