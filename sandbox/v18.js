...
// Lungitz sandbox v18 — find-or-reuse masthead menu. Forked from production
// (the promoted v17). The masthead menu now READS its items from a Designer-
// authored .nav-menu — two CMS Collection Lists bound to the Menu Entries
// collection (.nav-panel.is-giveaways / .is-hideaways), each item carrying a
// .nav-item label (→ Name) + a hidden .nav-item-body (→ Body). When that menu is
// present the script wires the reveal and builds nothing; otherwise it falls back
// to the in-code arrays. Runs on /sandbox (loaded by the loader) — so NO bail here.
// Motion + grid-rows transitions stay injected (Webflow's audit blocks them).
...
(function navMenu() {
    ...
    // ── Find-or-reuse the menu body ─────────────────────────────────────────
    // Live world: a Designer-authored .nav-menu holding two CMS Collection Lists
    // (.nav-panel.is-giveaways / .is-hideaways) whose items each carry a visible
    // .nav-item label (→ Name) + a hidden .nav-item-body (→ Body). Find them and
    // wire the reveal — build nothing. Fallback (no CMS menu): build the panels
    // from the arrays above so the menu still works everywhere. detail/detailBody
    // are shared with closeMenu below; ensureDetail sets them in both paths.
    ...
    // The Designer's .nav-body ships eye-hidden so the unstyled menu doesn't
    // sprawl on the canvas (its drawer layout only exists at runtime, injected
    // above). Lift the design-time hide here, whichever way Webflow encoded it.
    ...
    if (cmsMenu && cmsMenu.querySelector('.nav-item')) {
        // Reuse the CMS-rendered menu: wire each item's click to the reveal. The
        // body is the .nav-item-body sibling inside the same Collection List item.
