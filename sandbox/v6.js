...
(function injectCSS() {
    var INK = 'var(--_lungitz---color-ink-900)',
        css = [
            ...
            '  opacity:0;',
            '}',
            // v5 / ENGAGED STATE (sandbox prototype — final styling goes to the
            // Designer: mirror these onto the existing :hover selector groups).
            // Open entry holds the rust "engaged" look (mirror of :hover) instead
            // of depending on the cursor.
            '.trigger-accordion.open{',
            '  border-color:var(--_lungitz---color-accent-b-500);',
            '  color:var(--_lungitz---color-accent-b-500);',
            '  border-radius:8px;',
            '}',
            // The thumbnail you were viewing stays revealed (veil lifted) until
            // you click away.
            '.wrapper-thumbnail.is-revealed .thumb-hover{',
            '  background-color:transparent;',
            '  -webkit-backdrop-filter:none;backdrop-filter:none;',
            '}',
            // v6 / LIGHTBOX ZONES (Option 1: edge-strip nav + backdrop close).
            // Invisible click targets; the image stays click-to-zoom in the
            // centre. Stacking: close zone (1) < image (2) < nav strips (3) <
            // controls (5), so margin clicks close, image clicks zoom, and
            // edge clicks page — even over a wide image.
            '.lb-zone{position:absolute;z-index:3}',
            '.lb-zone-close{inset:0;z-index:1}',
            '.detail-view.is-fullscreen ' + DETAIL_IMG + '{position:relative;z-index:2}',
            '.lb-zone-prev{left:0;top:15%;width:18%;height:70%;cursor:pointer}',
            '.lb-zone-next{right:0;top:15%;width:18%;height:70%;cursor:pointer}',
            '.detail-view.is-fullscreen [data-detail],',
            '.detail-view.is-fullscreen .caption-drawer{position:relative;z-index:5}',
            '.lb-chevron{position:absolute;top:50%;transform:translateY(-50%);',
            '  color:var(--_lungitz---color-paper-000,#fff);font-size:40px;',
            '  line-height:1;opacity:0;transition:opacity .2s ease;',
            '  pointer-events:none;text-shadow:0 1px 4px rgba(0,0,0,.5)}',
            '.lb-zone-prev .lb-chevron{left:16px}',
            '.lb-zone-next .lb-chevron{right:16px}',
            '.lb-zone-prev:hover .lb-chevron,',
            '.lb-zone-next:hover .lb-chevron{opacity:.85}',
            // Zoomed: disable zones so click = zoom-out and drag = pan.
...
function openDetail(trigger, idx, images) {
    ...
    // Fresh engagement: clear any previously-revealed thumbnail in this entry.
...
function closeDetail(trigger) {
    ...
    if (detail && detail.trigger === trigger) {
        // Keep the thumbnail you were viewing revealed (veil lifted) on return.
        var thumbs = trigger.querySelectorAll(W_THUMB);
        if (thumbs[detail.idx]) {
            thumbs[detail.idx].classList.add('is-revealed');
        }
        detail = null;
    }
}
...
function buildZones(view) {
    if (view.querySelector('.lb-zone')) {
        return;
    }
    var close = document.createElement('div');
    close.className = 'lb-zone lb-zone-close';
    var prev = document.createElement('div');
    prev.className = 'lb-zone lb-zone-prev';
    prev.innerHTML = '<span class="lb-chevron">‹</span>';
    var next = document.createElement('div');
    next.className = 'lb-zone lb-zone-next';
    next.innerHTML = '<span class="lb-chevron">›</span>';
    view.appendChild(close);
    view.appendChild(prev);
    view.appendChild(next);
}

function removeZones(view) {
    view.querySelectorAll('.lb-zone').forEach(function (z) {
        z.parentNode.removeChild(z);
    });
    view.classList.remove('lb-zoomed');
    view.classList.remove('lb-single');
}
...
function openFullscreen() {
    ...
    buildZones(view);
    if (detail && detail.images.length === 1) {
        view.classList.add('lb-single');
    }
...
function closeFullscreen() {
    ...
    // ── v3 / OPTION C: single-image close snaps instantly, no motion ──
    // Bulletproof: leave fullscreen and restore the preview in one synchronous
    // task, no transition. No animation means no flash, but also no shrink feel.
    if (wasSingle) {
        view.style.transition = 'none';
...
function zoomIn(img, clientX, clientY) {
    ...
    if (fs) { fs.view.classList.add('lb-zoomed'); }
}
...
function zoomOut(img) {
    ...
    if (fs) { fs.view.classList.remove('lb-zoomed'); }
    zoom = null;
    panState = null;
    setTimeout(function () {
        if (!zoom) {
...
function resetZoom() {
    ...
    fs.view.classList.remove('lb-zoomed');
    zoom = null;
    panState = null;
}
...
// Lightbox trigger zones: edge strips page prev/next, backdrop closes.
// Disabled while zoomed (zones get pointer-events:none, but guard anyway).
document.addEventListener('click', function (e) {
    var z;
    if (!fs || zoom) {
        return;
    }
    z = e.target.closest('.lb-zone');
    if (!z) {
        return;
    }
    e.preventDefault();
    if (z.classList.contains('lb-zone-close')) {
        closeFullscreen();
        return;
    }
    if (!detail) {
    ...
    if (z.classList.contains('lb-zone-prev')) {
        detail.idx = (detail.idx - 1 + detail.images.length) % detail.images.length;
        paintDetail(fs.view);
    } else if (z.classList.contains('lb-zone-next')) {
