...
// ════════════════════════════════════════════════════════════════════════
//  Lungitz sandbox v26 — fresh path. Cumulative (v25) + the LINE-HEIGHT slip fix.
//
//  Bug Seth caught: .author bump-drops on the state 2→1 collapse. Cause: .author
//  has line-height:var(--space-5) — a SIZE token used for leading. Phase 2 makes
//  space-5 fluid (clamp w/ vw), so the leading became viewport-dependent and jumps
//  when the collapse toggles a scrollbar (vw shifts). Same class of bug fixed in
//  Phase 1 for .title/.h5-nav. Six classes bind line-height to a space token:
//    .author .rich-text-block .type → space-5 ;  .button .button-copy → space-8 ;
//    .number-list → space-4 (kept fixed, harmless, rebound anyway for hygiene).
//  Fix: rebind each to a unitless line-height token (tight/normal/loose) — stable,
//  on-system, no vw. (Exact ratios are Seth's to tune in the Designer.)
//  NOTE: this rebinding is REQUIRED in the Phase 2 promotion (else the bug ships).
//
...
var LH = function (k) { return 'var(--_lungitz---line-height-' + k + ')'; };  // tight/normal/loose
...
var css =
  ...
  + '}'
  // ── LINE-HEIGHT slip fix: space token → unitless line-height token (no vw, no bump) ──
  + '.author{line-height:' + LH('normal') + '!important;}'        /* was space-5 (~1.43) */
  + '.rich-text-block{line-height:' + LH('tight') + '!important;}' /* was space-5 (~1.11) */
  + '.type{line-height:' + LH('normal') + '!important;}'
  + '.number-list{line-height:' + LH('normal') + '!important;}'
  + '.button,.button-copy{line-height:' + LH('loose') + '!important;}'  /* was space-8 (~1.78) */
  // (1) immersive masthead: HIDEAWAYS justifies hard-right (✕-gap gone)
