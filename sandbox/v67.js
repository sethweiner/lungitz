...
//   height attribute, so until each decodes it occupies nothing and the entry's
//   height jumps as they land. Reserving space is the durable fix and is Seth's
//   call; fetching on hover/press is what code can do without touching the design.
// v59 (2026-07-26) — ONE SCROLL MECHANISM. The click path still used the morning's
//   "pick a scroller, tween its scrollTop" code while arrivals had moved to the
//   error-paydown walk. In a single column that pick was wrong, so clicking
//   GIVEAWAYS moved the page by nothing at all (measured: scrollY 2200 before and
//   after). Both paths now ease the SAME way — consume a share of the current
//   error each frame, take the remainder on the last — so it lands exactly in one
//   column, in two, and mid-load, and there is no second path left to disagree.
// v58 (2026-07-26) — code hands the column's top padding BACK to the Designer.
//   v56/v57 wrote it from a measured masthead, which was wrong twice: it clobbered
//   a Designer value, and it measured a masthead still growing with the webfonts,
//   so it wrote 60px where 81 was wanted. The clearance for the first item in a
//   column is a layout constant, not something to chase at runtime. Diagnosis and
//   the exact value are in MASTHEAD-CONTRACT §9.
// v57 (2026-07-26) — and the measurement is re-read while the TYPE settles. The
//   masthead's height is type-driven, so it grows as the webfonts land: 37px early,
//   58px settled — a pad of 60 then 81. Reading it at a few named moments left the
//   column padding stuck on the early value while the anchor had moved on, which is
//   exactly the residual off_by. Re-read across the first seconds, and on every
//   arrival tick, so pad, padding and scroll all come from one measurement.
// v56 (2026-07-26) — THE FIRST ITEM IN A COLUMN CANNOT BE SCROLLED DOWN. That is
//   why "the anchor doesn't land" survived every scroll fix: #info-* are the first
//   elements in their columns, already at scrollTop 0, so their clearance is the
//   column's padding-top and nothing else. That padding is 8vh — viewport-driven —
//   while the masthead's height is type-driven; at 1280x720 it came out 50.4px
//   against a masthead ending at 58, leaving the heading 8px underneath it.
//   The column's top padding is now raised to the same measured pad the anchor
//   uses (raised only, never reduced), so layout and anchor agree by construction.
// v55 (2026-07-26) — STOP CHOOSING A SCROLLER. Which element scrolls is not a fact
//   about the breakpoint, it is a fact about the layout at that instant, and during
//   load the page passes through states that look like the other layout. The
//   ?lzdebug readout caught it on DESKTOP: the document was briefly tall, the
//   arrival committed to it and scrolled 5051px, then the two-column layout
//   established, that scroll ceased to exist and the block sat 31px under the
//   masthead (off_by=-31). Arrivals now measure the error and pay it down against
//   whatever will absorb it, ancestors first, document last.
// v54 (2026-07-26) — the arrival HOLDS its position instead of settling early.
//   Two identical readings 240ms apart proved only that nothing moved in that
//   window; with 215 lazy images still decoding it is usually a lull between two
//   of them. That is why it landed and then slid. Keep re-applying for the whole
//   window (cheaply once quiet); a no-op when already correct, cancelled the
//   moment the reader scrolls.
// v53 (2026-07-26) — STOP GUESSING ACROSS THE DEVICE GAP. This pane reports
//   document.hidden permanently and disagrees with a real phone, and inferring
//   across that gap has cost more time than the bugs. Adds a flight recorder
//   (always on, a few lines) and an opt-in ?lzdebug=1 readout so one screenshot
//   from the device answers what is actually happening. Also re-opens the arrival
//   poll when a late image lands — every CMS thumbnail is lazy with no height
//   attribute, so it reserves no space and shifts whatever is below it on decode.
// v52 (2026-07-26) — the #info arrival is OURS, and mobile stops animating layout.
//   · The head gate now strips the #info hash before <body> parses, so the browser
//     never jumps. There is then no wrong position to correct — only a right one to
//     set, once the layout has stopped moving. Polled (setTimeout survives a
//     backgrounded tab where rAF and ResizeObserver do not), settles when the
//     target reads the same twice, restores the hash so the URL stays shareable.
//   · Expand no longer transitions grid-template-rows at <=767px: that re-runs
//     layout every frame across one very long stacked document with 215 lazy
//     images, which is the ratchet and the lag. Colour/border still ease.
// v51 (2026-07-26) — and the pad stops demanding position:fixed. The masthead is
//   `fixed` only where the document scrolls; on desktop it is `absolute` (the
//   columns scroll under it), so anchorPad() had been falling back to the old 64
//   constant there the whole time while the masthead measured 58. Test that it is
//   at the top of the scrollport instead, which is true in both modes.
// v50 (2026-07-26) — the measured pad reaches BOTH scrollers. The head block pins
//   scroll-padding on html and .wrapper-content; only html was being refreshed, so
//   the desktop columns kept the stale 64px while the masthead measured 58px there.
//   Also re-applied on load, since the pad is read before the type has settled.
// v49 (2026-07-26) — the expand fix keys off WHICH ELEMENT SCROLLS, not the input
//   device. v48 gated it on (pointer: coarse), which is right on a phone but wrong
//   in principle and untestable in a desktop pane — a narrow desktop window stacks
//   the columns and ratchets identically. Now: if the column is not a scroller, the
//   page is, so open in place and settle once afterwards.
// v48 (2026-07-26) — MOBILE CLOSE-OUT (Seth's list):
//   · native #hash jumps now honour the MEASURED masthead: scroll-padding-top was
//     pinned at 64px in the head block, a desktop constant, so the browser parked
//     info blocks wrong however the script behaved afterwards. Published from
//     anchorPad() and refreshed on resize/orientation/fonts.
//   · entry expand no longer ratchets on touch: the 620ms scroll-follow was chasing
//     a target still growing under it. Coarse pointers open in place and settle once,
//     instantly, after the transition ends. Desktop keeps the follow.
//   · fullscreen swipe claims horizontal gestures on touch (touch-action) and can no
//     longer be orphaned by pointercancel.
//   · blue tap flash killed (-webkit-tap-highlight-color).
// v47 (2026-07-26) — a dropped entry always loses its FLIP offset. drop() cleared the
//   transform inside requestAnimationFrame, which does not run in a backgrounded tab,
//   so a drop landing as the reader switches away stranded the entry at its drag
//   offset permanently. Timer fallback; same family as the orphaned ghost.
...
function lzLog(what, data) {
    try {
        window.__lzTrace.push({ t: Date.now(), what: what, data: data });
        if (window.__lzTrace.length > 60) { window.__lzTrace.shift(); }
    } catch (e) {}
}
window.addEventListener('error', function (e) {
    lzLog('ERROR', (e.message || '') + ' @' + (e.filename || '').slice(-28) + ':' + (e.lineno || ''));
});
...
(function injectCSS() {
    var INK = 'var(--_lungitz---color-ink-900)',
        css = [
            // NO BLUE TAP FLASH (v48). Android and iOS paint a translucent highlight
            // over whatever you tap; on the entry images it read as the element
            // flashing blue. It is a touch-only affordance the design already answers
            // with its own states, and Webflow cannot author the property.
...
function anchorPad() {
    var FALLBACK = 64, BREATH = 23,        // 64 = what 41px of masthead + 23 used to give
        mast = document.querySelector('.nav.wide'), r;
    if (!mast) { return FALLBACK; }
    r = mast.getBoundingClientRect();
    // The masthead pins to the top BOTH ways and the position value differs by
    // breakpoint: `fixed` where the document scrolls (mobile), `absolute` where it
    // does not (desktop — the columns scroll under it instead). Requiring `fixed`
    // therefore made this silently fall back to the old constant on desktop, where
    // the masthead actually measures 58px, so entries settled ~17px too high, half
    // under it. Ask only whether it is really sitting at the top of the scrollport:
    // that is true in both modes, and false when the landing modal is open and this
    // same element is the full-height frame.
    if (!r.height || r.bottom < 0 || r.bottom > 200) { return FALLBACK; }
    return Math.round(r.bottom + BREATH);
}

// Does opening this entry mean scrolling the WHOLE PAGE? On desktop each column is
// its own scroll container, so following an opening entry moves only that column and
// nothing else shifts. Once the columns stack, the document is the scroller and the
// same follow drags the entire page while the entry is still growing — which is the
// ratchet. Ask the layout, not the input device: a narrow desktop window has exactly
// the same problem as a phone, and this is testable at any width.
function pageIsScroller(el) {
    var col = el.closest('.wrapper-content');
...
// which thing scrolls.
//
// Deciding was the bug. Whether the column or the document scrolls is not a fact about
// the breakpoint, it is a fact about the layout AT THIS INSTANT — and during load the
// page passes through states that look like the other one. On desktop the document is
// briefly tall enough to scroll before the columns take their height, so an arrival
// committed to the document, scrolled it 5051px, and then the two-column layout
// established, that scroll ceased to exist, and the block sat 31px under the masthead.
// Seth: "you're crossing wires from the two-column to the one-column layout."
//
// So measure the error and pay it down against whatever will actually take it: walk the
// ancestors, give each scrollable one as much of the remaining delta as it can absorb,
// finish at the document. Right in one column, in two, and in the moments in between —
// and because it works from the CURRENT error, repeating it simply converges.
function settleExactly(el, pad, portion) {
    var delta = (el.getBoundingClientRect().top - pad) * (portion === undefined ? 1 : portion),
        node = el.parentElement, before;
    function give(target) {
        if (!target || Math.abs(delta) < 1) { return; }
        before = target.scrollTop;
        target.scrollTop = before + delta;
        delta -= (target.scrollTop - before);      // only what it actually took
    }
    ...
    give(document.scrollingElement || document.documentElement);
}

function scrollToTrigger(trigger, instant) {
    var pad = anchorPad(), DUR = 620, start = null;
    if (instant) {
        settleExactly(trigger, pad);
        return;
    }
...
function openFullscreen() {
    ...
    // GROWTH IN (Seth, 2026-07-25: "growth in, growth out — restore the old
    // feel"): the WHOLE overlay FLIP-morphs from the clicked THUMBNAIL's rect to
    // the full frame — the single-image path's mechanics, now the only path
    // (v66: state 3 is cut, so the thumbnail is always the source). The rest
    // state is a static hide (opacity 0, clipped).
    // NO SQUEEZE (v67): the morph used to scale non-uniformly (frame aspect →
    // thumbnail aspect), visibly distorting the image with mixed-ratio thumbs.
    // Now the scale is UNIFORM — the factor that makes the frame COVER the
    // thumbnail rect — and an animated clip-path inset crops the rest. The
    // aspect difference is absorbed by cropping (exactly how the thumbnail
    // itself presents its image), never by stretching. Same token, same clock.
    ...
    if (srcRect && srcRect.width) {
        var m = lzFlipMath(OVERLAY.getBoundingClientRect(), srcRect);
        OVERLAY.style.transition = 'none';
        OVERLAY.style.transform = m.transform;
        OVERLAY.style.clipPath = m.clip;
        requestAnimationFrame(function () {
            OVERLAY.style.transition = 'transform ' + TRANSITION + 'ms ' + SETTLE
                                     + ', clip-path ' + TRANSITION + 'ms ' + SETTLE;
...
// The uniform FLIP (v67): map a full-frame rect onto a target rect with ONE scale
// factor (the one that covers the target) plus a centred clip window that crops
// the overflow — in the element's local, pre-transform pixels. Shared by growth
// in (start state) and growth out (end state), so both directions are the same
// gesture and the image never changes shape.
function lzFlipMath(frame, target) {
    var s = Math.max(target.width / frame.width, target.height / frame.height),
        dx = target.left + target.width / 2 - (frame.left + frame.width / 2),
        dy = target.top + target.height / 2 - (frame.top + frame.height / 2),
        ix = Math.max(0, (frame.width - target.width / s) / 2),
        iy = Math.max(0, (frame.height - target.height / s) / 2);
    return {
...
function closeFullscreenNow() {
    ...
    // GROWTH OUT (v66) — the overlay FLIP-shrinks to the thumbnail of the image
    // being VIEWED (thumbs[detail.idx]) — not the one that opened it: cycling
    // with the chevrons moves the landing pad, which is T-02's fix. It eases
    // back INTO place on the SETTLE token over TRANSITION ms — the exact
    // gesture of the grow, reversed (Seth: "shrinking needs some easing back
    // into place … this relation to the easing tokens"). Option-C's snap is
    // retired: the thumbnail is always an in-flow target now, and v62 reserves
    // its footprint, so there is always a stable rect to settle into.
    ...
    if (!tRect || !tRect.width) {                       // target vanished → static hide
    ...
    // v67: uniform scale + animated clip (see lzFlipMath) — the shrink crops,
    // never stretches, mirroring the grow exactly.
    var m = lzFlipMath(view.getBoundingClientRect(), tRect);
    view.style.clipPath = 'inset(0px 0px 0px 0px)';
    void view.offsetHeight;
    view.style.transition = 'transform ' + TRANSITION + 'ms ' + SETTLE
                          + ', clip-path ' + TRANSITION + 'ms ' + SETTLE;
    view.style.transform = m.transform;
    view.style.clipPath = m.clip;
    setTimeout(function () {
...
document.addEventListener('click', function (e) {
    ...
    // NO SCROLL-FOLLOW WHEN THE PAGE ITSELF SCROLLS (v48/v49). Tweening the page for
    // 620ms while the entry's own grid-rows transition is still growing beneath it
    // means every frame re-measures a target that has moved — on mobile, where the
    // DOCUMENT is the scroller, that reads as the expand advancing in steps
    // ("ratcheted", Seth). Desktop is unaffected: there the column scrolls, the page
    // does not, and the follow is the point. Where the page scrolls we let the entry
    // open in place and settle it once, instantly, after the transition has finished.
    if (hadOpen) {
...
// ── Give an entry's images a head start (v60) ───────────────────────────────
// Why the expand ratchets: .wrapper-thumbnail has NO width or height of its own —
// the strip is sized entirely by whatever the image turns out to be, and every
// thumbnail is loading="lazy" with no height attribute. Until each one decodes it
// occupies nothing, so the entry's height jumps as they arrive, during and after the
// transition. Animating a box whose contents keep resizing is what "ratchet and lag"
// looks like, and no easing can smooth it.
// The durable fix is reserving space on the thumbnails — a Designer decision, since it
// sets what the strip looks like before it fills. What code can do without touching the
// design is stop the images arriving LATE: start fetching them on hover, and on press,
...
(function arrange() {
    ...
    function drop() {
        ...
        // The settle animation is a nicety; CLEARING the FLIP offset is not. rAF does
        // not run in a backgrounded tab, so a drop that lands as the reader switches
        // away would leave the entry stranded at its drag offset for good — the same
        // family as the orphaned ghost. Guarantee the reset on a timer no matter what.
        function clearFlip() { node.style.transition = ''; node.style.transform = ''; }
        requestAnimationFrame(function () {
            node.style.transition = 'transform 450ms ' + SETTLE;
            node.style.transform = '';
            setTimeout(clearFlip, 470);
        });
        setTimeout(clearFlip, 700);
    }
...
(function slideshowNav() {
    ...
    // A swipe must never be left half-done (v48). The browser fires pointercancel
    // instead of pointerup whenever it claims the gesture; without this the image
    // keeps its drag offset and transition:none for good — the same orphaning that
    // stuck entries to the viewport.
    document.addEventListener('pointercancel', function () {
        if (!swipe) { return; }
        swipe.img.style.transition = '';
        swipe.img.style.transform = 'none';
        swipe = null;
    });
}());
...
// Keep the browser's OWN anchor jumps in step with the measured masthead (v48).
// Still worth doing for any anchor we do not intercept.
var lzSyncPad = null;
(function syncScrollPadding() {
    function apply() {
        try {
            var pad = anchorPad() + 'px';
            // AND make sure the first item in a column can clear the masthead at all.
            // This was the real bug behind "the anchor doesn't land": #info-giveaways
            // and #info-hideaways are the FIRST elements in their columns, already at
            // scrollTop 0, so no amount of scrolling can push them further down — their
            // clearance is the column's padding-top and nothing else. That padding is
            // 8vh, which tracks the VIEWPORT, while the masthead's height tracks the
            // TYPE; at 1280x720 they came out 50.4px against a masthead ending at 58,
            // so the info heading sat 8px underneath it and every scroll fix was
            // arguing with geometry. Raise it to the same measured pad the anchor uses,
            // so layout and anchor agree by construction. Only ever RAISES it — Seth's
            // spacing wins wherever it is already generous enough.
            // NOTE: code does NOT touch the column's padding-top. It tried in v56/v57 and
            // that was wrong twice over — it overwrote a Designer value with a number
            // measured from a masthead that is still growing as the webfonts land, so it
            // wrote 60px where 81px was needed and left Seth's 8vh clobbered by a guess.
            // The clearance for the FIRST item in a column is a layout constant and
            // belongs on the class: `.wrapper-content` padding-top must be >= the
            // masthead's height plus breathing room. It is currently 8vh, which scales
            // with the VIEWPORT while the masthead scales with the TYPE — 50.4px against
            // a masthead ending at 58 at 1280x720. That mismatch is the whole bug, and
            // no amount of scrolling can paper over it: #info-* sit at scrollTop 0 and
            // cannot be pushed down. See MASTHEAD-CONTRACT §9.
            document.querySelectorAll('.wrapper-content').forEach(function (c) {
                c.style.scrollPaddingTop = pad;
                c.style.paddingTop = '';           // hand it back to the Designer
            });
    ...
    // The masthead's height is TYPE-driven, so it keeps changing while the webfonts
    // land — measured 37px early and 58px settled, i.e. a pad of 60 then 81. Reading
    // it only at a handful of named moments left the column padding stuck on the early
    // value while the anchor had already moved on, which is precisely the residual
    // "off_by". Re-read for the first few seconds so every consumer agrees.
    [150, 400, 800, 1500, 2500, 4000].forEach(function (d) { setTimeout(apply, d); });
}());

// ── #info arrival, ours from start to finish (v52) ──────────────────────────
// The head gate stripped the hash before <body> parsed, so the browser never jumped
// and the page is simply at the top: there is no wrong position to correct, only a
// right one to set. That is the whole point. Every previous attempt was trying to
// out-argue a jump the browser had already made at parse time, before the fonts and
// the lazy CMS images reflowed everything above the target — ~100px of drift that no
// amount of re-correcting caught reliably.
// Poll instead of observing: setTimeout keeps running when rAF and ResizeObserver do
// not, so this behaves the same in a backgrounded tab, and it is testable. Stop as
// soon as the target reads the same twice in a row — that is the layout confirming it
// has settled — or on any real scroll, because the reader has taken over.
(function infoArrival() {
    var id = window.__lzInfoTarget;
    if (!id) { return; }
    var el = document.getElementById(id);
    if (!el) { return; }
    var last = null, stable = 0, done = false, timer = null,
    ...
    function stop(withHash) {
        if (done) { return; }
        done = true;
        clearTimeout(timer);
        window.removeEventListener('wheel', abandon, true);
        window.removeEventListener('touchstart', abandon, true);
        window.removeEventListener('keydown', abandon, true);
        window.removeEventListener('load', onLateImage, true);
        if (withHash) { restoreHash(); }
    }
    function abandon() { stop(true); }          // reader took over — keep the URL honest

    function tick() {
        if (done) { return; }
        if (el.offsetParent !== null) {
            if (lzSyncPad) { lzSyncPad(); }     // pad, padding and scroll from one measurement
            scrollToTrigger(el, true);
            var top = Math.round(el.getBoundingClientRect().top);
            stable = (top === last) ? stable + 1 : 0;
            last = top;
            lzLog('arrive.tick', { top: top, stable: stable, pad: anchorPad(), y: Math.round(window.scrollY) });
        } else {
            lzLog('arrive.hidden', null);
        }
        // DO NOT STOP EARLY. Reading the same value twice only proves nothing moved in
        // the last 240ms — and with 215 lazy images still decoding, that is routinely a
        // lull between two of them, not the end of the reflow. Stopping there is why it
        // landed correctly and then slid afterwards. Hold the position for the whole
        // window instead; once it is already right, re-applying is a no-op, and the
        // reader taking over cancels it immediately.
        if (Date.now() > deadline) {
            lzLog('arrive.settled', { top: last, reason: 'window ended' });
            stop(true);
            return;
        }
        timer = setTimeout(tick, stable >= 2 ? 300 : 120);   // ease off once quiet
    }

    // A late image is the thing most likely to move the target after we have settled:
    // every CMS thumbnail is loading="lazy" with no height attribute, so it reserves
    // no space and shifts whatever sits below it the moment it decodes. Two identical
    // readings 120ms apart can happen in a gap between two images arriving. Listening
    // for load in the capture phase catches exactly that and re-opens the poll.
    function onLateImage() {
        if (done || Date.now() > deadline) { return; }
        stable = 0;
        lzLog('arrive.reflow', 'image loaded');
        clearTimeout(timer);
        timer = setTimeout(tick, 60);
    }
    window.addEventListener('load', onLateImage, true);   // capture: fires for <img> too

    window.addEventListener('wheel', abandon, { capture: true, passive: true });
    window.addEventListener('touchstart', abandon, { capture: true, passive: true });
    window.addEventListener('keydown', abandon, true);
    lzLog('arrive.start', { id: id, pad: anchorPad() });
    tick();
}());

// ── ?lzdebug=1 — an on-screen readout, for a device I cannot inspect ────────
// Strictly opt-in: without the parameter not one line of this runs and nothing is
// added to the page. It exists because this pane and a real phone keep disagreeing,
// and one screenshot from the phone settles more than an afternoon of inference.
(function debugPanel() {
    if (!/[?&]lzdebug=1\b/.test(location.search)) { return; }
    var box = document.createElement('pre');
    box.style.cssText = 'position:fixed;left:0;right:0;bottom:0;z-index:2147483647;margin:0;'
        + 'max-height:52vh;overflow:auto;background:rgba(0,0,0,.92);color:#8fe;'
        + 'font:10px/1.35 ui-monospace,Menlo,monospace;padding:8px;white-space:pre-wrap;'
        + 'border-top:1px solid #8fe;-webkit-user-select:text;user-select:text';
    function ready() {
    ...
    function draw() {
        var mast = document.querySelector('.nav.wide'),
            mr = mast ? mast.getBoundingClientRect() : null,
            trig = document.querySelector('.trigger-accordion'),
            id = window.__lzInfoTarget,
            info = id ? document.getElementById(id) : null,
            ir = info ? info.getBoundingClientRect() : null,
            L = [];
        L.push('LUNGITZ ' + LZ_VERSION + '  ' + innerWidth + 'x' + innerHeight + ' dpr' + (devicePixelRatio || 1));
        L.push('mq<=767 ' + matchMedia('(max-width:767px)').matches
             + '   pageScrolls ' + (document.documentElement.scrollHeight > innerHeight));
        L.push('masthead ' + (mast ? getComputedStyle(mast).position : '?')
             + ' bottom=' + (mr ? Math.round(mr.bottom) : '?')
             + '   anchorPad=' + anchorPad());
        L.push('scrollPad html=' + document.documentElement.style.scrollPaddingTop
        ...
        L.push('scrollY=' + Math.round(window.scrollY) + '  imgs_pending='
             + [].filter.call(document.images, function (i) { return !i.complete; }).length);
        L.push('— trace —');
        var t0 = (window.__lzTrace[0] || {}).t || Date.now();
        window.__lzTrace.slice(-16).forEach(function (e) {
            L.push('+' + String(e.t - t0).padStart(5) + 'ms ' + e.what + ' '
