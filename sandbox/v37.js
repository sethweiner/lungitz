...
function scrollToTrigger(trigger) {
    ...
    function step() {
        ...
        if ((Math.abs(diff) < 1.5 && Math.abs(target - smoothed) < 1.5) || frames > maxFrames) {
            // LAND EXACTLY (v37). The easing closes ~8% of the remaining gap per
            // frame and gives up after maxFrames, so where an entry came to rest
            // depended on how far it had to travel: a short hop landed on the
            // mark, a long one stopped short (0.92^58 ≈ 0.8% of the gap, ~16px
            // over a 2000px scroll). That is the inconsistent anchor — the same
            // entry settled differently depending on where you clicked from.
            // target is recomputed every frame, so this snaps to the live value.
