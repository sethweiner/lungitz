...
function closeFullscreen() {
    ...
    // ── v2 / OPTION A: single-image close uses the normal FLIP path ──
    // No thumbnail handoff. The image shrinks from fullscreen back to the
    // in-flow detail view and STAYS as the resting (preview) state — there
    // is no swap to a separate thumbnail element, so nothing can flash.
    // (wasSingle is intentionally not special-cased; it falls through.)
