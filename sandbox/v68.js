...
// Feel candidates for the fullscreen morphs (v68) — existing tokens only,
// switched from the ARRIVAL url (INITIAL_SEARCH — never the live one). Seth
// picks by feel: default keeps the SETTLE token; ?ease=2 the closing token;
// ?ease=3 the closing token at 350ms.
var FS_EASE = (function () {
    var m = /[?&]ease=([23])\b/.exec(INITIAL_SEARCH);
    if (m && m[1] === '2') { return { dur: TRANSITION, curve: CLOSE_EASE }; }
    if (m && m[1] === '3') { return { dur: 350, curve: CLOSE_EASE }; }
    return { dur: TRANSITION, curve: SETTLE };
}());
