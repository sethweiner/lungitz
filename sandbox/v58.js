...
function settleExactly(el, pad) {
    var delta = el.getBoundingClientRect().top - pad,
...
function scrollToTrigger(trigger, instant) {
    ...
    if (instant) {
        settleExactly(trigger, pad);   // arrivals correct themselves, they don't perform
        return;                        // (arrive()'s precedent: a slide on load reads as jumpy)
    }
