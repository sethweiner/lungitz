...
| v18 | **Menu as real elements (CMS)** | Production + **find-or-reuse `navMenu`**: the masthead menu now lives in the Designer — `.nav-body` → `.nav-menu` → two **Menu Entries** Collection Lists (`.nav-panel.is-giveaways` / `.is-hideaways`), each item a `.h5-nav.nav-item` label (→ Name) + hidden `.nav-item-body` (→ Body). The script finds that structure, wires click→`.nav-detail` reveal, and **builds nothing**; falls back to the in-code arrays if the CMS menu is absent (Home keeps working pre-promotion). `.nav-body` ships **eye-hidden** in the Designer (tidy canvas); the script lifts the hide at runtime. Drawer motion stays injected. |
...
| v32 | **Landing/menu modal concept** | Seth's 2026-07-25 wireframes made live. **§landingModal**: the `container-landing` modal IS the landing + menu — greets on index arrival, any click outside its links dismisses in place, LUNGITZ re-opens it (dormant until instanced). **§masthead** (focused): GIVEAWAYS/HIDEAWAYS → `#info-giveaways`/`#info-hideaways` anchors + rust cue; `.category-content` "+" toggles `.is-expanded` (both states Designer-styled). **Fullscreen**: Seth's `.immersive-overlay` is the state-4 view when present (find-or-legacy) — paint (title "N · Entry", counter, captions), `.is-open` toggle, light image FLIP in, snap out; gestures retargeted via `.immersive-image, .detail-image`. **Browser BACK closes fullscreen** (history state — the Antoine fix; hint chip says "✕ / back" in browser-fullscreen). Participants links rewrite `/giveaways/<slug>` → `/?entry=…` (index arrival). RETIRED: drawer menu, veil, `?realm=`, is-immersive masthead frame. Contributors got auto-populated `featured-work-giveaway/hideaway` ref fields (35/37 matched). |
| v31 | **Nav concept redesign (17.6 feedback)** | Production (post-keyboard-arc) + the client round: **realm words go there** — GIVEAWAYS/HIDEAWAYS scroll + rust-light their column (instant, arrive() precedent; `/?realm=side` from other pages) instead of opening the menu; the menu moves to a Designer `.nav-menu-toggle` (interim: LUNGITZ toggles on the index); a `.nav-item` with a real href (Designer-bound to its Menu Entries template page) **navigates** — inline reveal stays as fallback; `landingVeil()` gate (dormant until Seth authors `.landing-veil`; click/⏎/Esc/scroll dismiss, scroll-lock, deep-link skip, `?veil=0`); mobile drawer scrolls when taller than the viewport (`max-height:100dvh−1.5rem` + momentum, breakpoint unified 640→767); fullscreen ✕ un-hidden on coarse pointers; external links → `_blank noopener`; LUNGITZ→home on any column-less page. See DESIGNER-HANDOFF_2026-07-24.md for Seth's wireframe list. |
