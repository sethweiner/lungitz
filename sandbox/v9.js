...
// Immersive frame (v9 — Track B): in fullscreen the masthead persists as the
// frame instead of being covered. Toggle .is-immersive on .nav (frame breathes +
// reveals the ✕), and light the active realm word (is-left → giveaways,
// is-right → hideaways) in the rust accent. The ✕ + styling live in navMenu.
function setImmersive(on, trigger) {
    var nav = document.querySelector('.nav.expand'),
        g, h, word;
    if (!nav) { return; }
    nav.classList.toggle('is-immersive', on);
    g = document.querySelector('.nav-giveaways .h5-nav');
    h = document.querySelector('.nav-hideaways .h5-nav');
    if (g) { g.classList.remove('is-active'); }
    if (h) { h.classList.remove('is-active'); }
    if (on && trigger) {
        word = trigger.closest('.wrapper-content.is-right') ? h
             : (trigger.closest('.wrapper-content.is-left') ? g : null);
...
(function navMenu() {
    ...
    var V = function (n) { return 'var(--_lungitz---' + n + ')'; },
        css = [
            ...
            // ── v9 immersive frame (Track B — fullscreen persistence) ──
            ...
            '.detail-view.is-fullscreen{inset:calc(4vh + 3rem) 1.5rem 1.5rem;}',
            ...
            // Fullscreen ✕ — hidden until immersive; tucks into the frame corner.
            '.frame-close{',
            '  display:none;position:absolute;top:0;bottom:0;right:' + V('space-2') + ';',
            ...
            '}',
            '.nav.expand.is-immersive .frame-close{display:flex;}',
