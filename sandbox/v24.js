...
// ════════════════════════════════════════════════════════════════════════
//  Lungitz sandbox v24 — fresh path. Cumulative: v23 (Phase 2 macro-space +
//  mobile masthead/scrollbar + fullscreen close A) with two Seth refinements:
//   · Close affordance: tooltip was "too much" → drop it; instead the CURSOR
//     becomes an ✕ whenever it's over an exit target (LUNGITZ + the backdrop).
//   · Prev/next: the 4rem .fs-chev is bulky → restyle it to the state-3 .button
//     look (transparent, accent-a ← / →, rust on hover). One restyle fixes BOTH
//     the fullscreen slideshow AND the CMS entry prev/next (.entry-nav), since
//     both use .fs-chev. (Entry-nav only shows on entry pages, not /sandbox —
//     verify there at promotion.)
//  Loads v18 (bounce-fixed). Phase 1 LIVE. [Keyboard-hotkeys arc still bookmarked.]
// ════════════════════════════════════════════════════════════════════════
...
// ✕ cursor (off-white, dark exit targets) — 26px, hotspot centred.
var XCUR = "url(\"data:image/svg+xml,%3Csvg%20xmlns='http://www.w3.org/2000/svg'%20width='26'%20height='26'%3E%3Cg%20stroke='%23e8e2da'%20stroke-width='2'%20stroke-linecap='round'%3E%3Cline%20x1='8'%20y1='8'%20x2='18'%20y2='18'/%3E%3Cline%20x1='18'%20y1='8'%20x2='8'%20y2='18'/%3E%3C/g%3E%3C/svg%3E\") 13 13, pointer";
...
var css =
  ...
  + '}'
  // ── Fullscreen close = LUNGITZ + backdrop + Esc; affordance = ✕ cursor ──
  + '.frame-close{display:none!important;}'
  + '.nav.wide.is-immersive .nav-lungitz{cursor:' + XCUR + ';}'
  + '.detail-view.is-fullscreen{cursor:' + XCUR + ';}'
  + '.caption-drawer.is-fullscreen{cursor:auto;}'   /* caption isn't an exit target */
  // ── Prev/next reborn as the state-3 .button arrows (kills the 4rem bulk) ──
