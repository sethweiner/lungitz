...
the Designer/CMS UI path, full srcset live (karl 5.7MB→787KB). Standing rule: **new
CMS images upload through the Designer/CMS UI only** (API/CSV = no responsive
variants), finish with SITE publish.
...
- Re-measure nav speed on the real domain (staging HTML adds ~2s).
